...
function Sprinkle(modelName) {
    $.getJSON("/Resource/?Resource=" + modelName , function (dataModelResource) {

        var dataModel = CreateDataModel(dataModelResource)

        $.getJSON("/Resource/Get/?Resource=" + modelName, function (data) {
        ...
        }).error(function () { alert("error with data"); })

    }).error(function () { alert("error resource"); })
